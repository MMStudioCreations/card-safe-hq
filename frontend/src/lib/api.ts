...
export const api = {
  ...
  sportsSearch: (q: string, sport?: string, page?: number, limit?: number) =>
    http.get<never, {
      data: Array<{
        id: string; name: string; set: string; number: string; year: string
        rarity: string; image: string; images: { small: string; large: string }
        prices: { low: string; mid: string; market: string; high: string }
        tcgplayer: { url: string }; condition: string; source: string
        ebayUrl: string; seller: string; card_type: string; sport: string
      }>
      totalCount: number; page: number; pageSize: number; query: string; sport: string
    }>('/api/sports/search', { params: { q, sport: sport ?? 'sports', page: page ?? 1, limit: limit ?? 20 } }),
  sportsSoldSearch: (q: string, sport?: string) =>
    http.get<never, { data: Array<Record<string, unknown>>; totalCount: number }>(
      '/api/sports/sold', { params: { q, sport: sport ?? 'sports' } }
    ),
